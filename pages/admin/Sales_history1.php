<?php
/**
 * Sales History Page - Branch Selection (Admin)
 * 
 * This page displays sales history for a selected branch with filtering capabilities.
...
$date_filter = isset($_GET['date_b1']) ? $_GET['date_b1'] : date('Y-m-d');

// Fetch all available branches for dropdown selection
$branches = [];
$branch_query = $conn->query("SELECT id, name FROM branches ORDER BY id");
while ($row = $branch_query->fetch_assoc()) {
    $branches[$row['id']] = $row['name'];
}

// Get selected branch ID from GET parameter, validate and default to branch 1
...
/**
 * Fetch sales records for the selected branch and date
...
if ($query_params) {
    // Build parameter types string (first param is integer, rest are strings)
    $param_types = str_repeat('s', count($query_params));
    $param_types[0] = 'i'; // branch_id is integer
...
if (!empty($sales)) {
    ...
    $sale_ids_placeholder = implode(',', array_fill(0, count($sale_ids), '?'));
    ...
    $items_sql = "SELECT si.*, 
                  ...
                  WHERE si.sale_id IN ($sale_ids_placeholder)";
...
// Calculate total sales for the selected date (NET of discount)
...
$stmt_sales->bind_param('is', $branch_id, $date_filter);
$stmt_sales->execute();
$stmt_sales->bind_result($total_sales_today);
$stmt_sales->fetch();
$stmt_sales->close();
// Align with whole-peso policy
$total_sales_today = floor($total_sales_today);

// Calculate total returns for the selected date
// Recompute returns with sale discounts applied proportionally
...
$stmt_returns->bind_param('is', $branch_id, $date_filter);
...
while ($return_row = $returns_result->fetch_assoc()) {
    ...
    // Cap returns for this sale to its net sale amount
...
// Floor returns to whole pesos before computing net sales
...
$stmt_product_returns->bind_param('is', $branch_id, $date_filter);
...
<head>
    ...
    <title>JohnTech Management System - Sales History</title>
...
<div class="container-fluid" style="margin-left: 250px !important; margin-top: 0 !important; padding: 0.5rem !important; padding-top: 0 !important; background: #f8fafc !important; min-height: 100vh !important; position: relative !important; z-index: 1 !important;">
    <div class="main-content" style="position: relative !important; z-index: 2 !important; margin-top: 0 !important; padding-top: 0.5rem !important;">
        ...
        <form method="get" class="mb-3" style="max-width:300px;">
            ...
            <!-- Do not carry date across branches to avoid cross-branch filtering -->
        </form>
        ...
        <h2 class="mb-4" style="color: #1a202c !important; font-size: 1.75rem;">
            <i class="bi bi-clock-history me-2"></i>Sales History - <?= htmlspecialchars($branches[$branch_id]) ?>
        </h2>
        ...
        <div class="content-card" style="background: #ffffff !important; border-radius: 12px !important; padding: 1.5rem !important; margin-bottom: 1.5rem !important; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1) !important; position: relative !important; z-index: 3 !important;">
            ...
            <!-- ===============================================================
                ...
                =============================================================== -->
            <div class="mb-3">
                <form method="get" action="Sales_history1.php" class="row g-2 align-items-end">
                    <!-- Date Filter Input -->
                    <div class="col-auto">
                        <label for="date" class="form-label mb-0">Select Date:</label>
                        <input type="date" id="date" name="date_b1" class="form-control" value="<?= htmlspecialchars($date_filter) ?>">
                    </div>
                    ...
                    <div class="col-auto">
                        <a href="?branch_id=<?= $branch_id ?>&date_b1=<?= date('Y-m-d') ?>" class="btn btn-success">
                            Today's Sales
                        </a>
                    </div>
                    ...
                    <div class="col-auto ms-auto text-end">
                        ...
                        <div class="dropdown ms-auto text-end">
                            <button class="btn btn-outline-secondary btn-sm dropdown-toggle" type="button" id="exportDropdown1" data-bs-toggle="dropdown" aria-expanded="false">
                                <i class="bi bi-download me-1"></i> Export
                            </button>
                            <div class="dropdown-menu dropdown-menu-end p-3" aria-labelledby="exportDropdown1" style="min-width:240px;">
                                <div id="exportControls1">
                                    <!-- Export Presets -->
                                    <div class="d-flex gap-2 mb-2 flex-wrap">
                                        <button type="button" id="preset_today" class="btn btn-outline-primary btn-sm">Today</button>
                                        <button type="button" id="preset_week" class="btn btn-outline-primary btn-sm">This Week</button>
                                        <button type="button" id="preset_month" class="btn btn-outline-primary btn-sm">This Month</button>
                                    </div>
                                    
                                    <!-- Hidden values for export -->
                                    <input type="hidden" id="exp_branch_id" value="<?= $branch_id ?>">
                                    <input type="hidden" id="exp_start_date" value="<?= $export_start ?>">
                                    <input type="hidden" id="exp_end_date" value="<?= $export_end ?>">
                                    <input type="hidden" id="exp_week_start" value="<?= $export_week_start ?>">
                                    <input type="hidden" id="exp_week_end" value="<?= $export_week_end ?>">
                                    <input type="hidden" id="exp_week_value" value="<?= $export_week_value ?>">

                                    <!-- Period Selection -->
                                    <div class="mb-2">
                                        <label class="form-label small mb-1">Period</label>
                                        <select id="exp_period" class="form-select form-select-sm">
                                    ...
                                    <div class="mb-2" id="exp1_date_wrap" style="display:none;">
                                        <label class="form-label small mb-1">Date</label>
                                        <input type="date" id="exp1_date" class="form-control form-control-sm">
                                    </div>

                                    <!-- Month Input (shown for monthly) -->
                                    <div class="mb-2" id="exp1_month_wrap" style="display:none;">
                                        <label class="form-label small mb-1">Month</label>
                                        <input type="month" id="exp1_month" class="form-control form-control-sm">
                                    </div>

                                    <!-- Week Input (shown for weekly) -->
                                    <div class="mb-2" id="exp1_week_wrap" style="display:none;">
                                        <label class="form-label small mb-1">Week</label>
                                        <input type="week" id="exp1_week" class="form-control form-control-sm" value="<?= $export_week_value ?>">
                                    </div>

                                    <!-- Date Range Input (shown for custom range) -->
                                    <div class="mb-2" id="exp1_range_wrap" style="display:none;">
                                        <label class="form-label small mb-1">Range</label>
                                        <div class="d-flex gap-2">
                                            <input type="date" id="exp1_start" class="form-control form-control-sm">
                                            <input type="date" id="exp1_end" class="form-control form-control-sm">
                                        </div>
                                    </div>
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <select id="exp_format" class="form-select form-select-sm">
                                    ...
                                    <!-- Export Button -->
                                    <button type="button" id="exportBtn1" class="btn btn-success btn-sm w-100">Export</button>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <?php else: ?>
                            <?php foreach ($sales as $sale): ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php
                                        $discount_amount = floatval($sale['discount']);
                                    ...
                                    <td>
                                        <?php if (isset($sales_with_returns[$sale['id']])): ?>
                                            <a href="view_returns.php?sale_id=<?= $sale['id'] ?>&branch_id=<?= $branch_id ?>" 
...
document.addEventListener('DOMContentLoaded', function() {
    var exportBtn = document.getElementById('exportBtn1');
    if (!exportBtn) return;
    
    /**
     * Helper function to build and open export URL
     * 
     * @param {string} period - Export period type (daily, weekly, monthly, range)
     * @param {string} start - Start date for export
     * @param {string} end - End date for export
     */
    function performExport(period, start, end) {
        var baseUrl = '<?= BASE_URL ?>/handlers/generate_sales_report.php';
        var branch = encodeURIComponent(document.getElementById('exp_branch_id').value || '');
        var format = encodeURIComponent(document.getElementById('exp_format').value || 'xlsx');
    ...
    function getSelectedWeekRange() {
        var start = document.getElementById('exp_week_start')?.value;
        var end = document.getElementById('exp_week_end')?.value;
        if (!start || !end) {
            return null;
        }
        return {
    ...
    exportBtn.addEventListener('click', function() {
        var period = document.getElementById('exp_period').value || 'monthly';
        var startDate = '';
        var endDate = '';
        
        if (period === 'daily') {
            // Use today's date for daily export
            startDate = '<?= date('Y-m-d') ?>';
            endDate = startDate;
        } else if (period === 'weekly') {
            var weekValue = document.getElementById('exp1_week').value || document.getElementById('exp_week_value').value;
            ...
            startDate = document.getElementById('exp_start_date').value || '';
            endDate = document.getElementById('exp_end_date').value || '';
        } else {
            // Custom range - validate input
            var startInput = document.getElementById('exp1_start').value;
            var endInput = document.getElementById('exp1_end').value;
     ...
     */
    var periodSelect = document.getElementById('exp_period');
    var dateWrap = document.getElementById('exp1_date_wrap');
    var weekWrap = document.getElementById('exp1_week_wrap');
    var monthWrap = document.getElementById('exp1_month_wrap');
    var rangeWrap = document.getElementById('exp1_range_wrap');
    var weekInputField = document.getElementById('exp1_week');
    ...
    if (weekInputField && !weekInputField.value) {
        weekInputField.value = document.getElementById('exp_week_value')?.value || '';
    }

    /**
     * Quick preset button handlers
     */
    var presetToday = document.getElementById('preset_today');
    ...
    if (presetMonth) {
        presetMonth.addEventListener('click', function() {
            var start = document.getElementById('exp_start_date').value || '';
            var end = document.getElementById('exp_end_date').value || '';
