...
// Fetch present mechanics for branch 1
$sql_mech = "SELECT id, name FROM mechanics WHERE branch_id = 1 ORDER BY name ASC";
...
<head>
    ...
    <title>Sorsogon Branch - POS</title>
...
<div class="container-fluid p-0 m-0">
    <div class="main-content">
        <h2 class="mb-2 mt-0 pt-0"><i class="bi bi-cart-check me-2"></i>POS (Over-the-Counter) - Sorsogon Branch</h2>
        <div class="content-card">
            <div class="row g-4">
                            ...
                            <tbody id="productTableBody">
                                ...
                                if (count($products) > 0):
                                    ...
                                    foreach ($products as $row):
                                        $next_category = null; // default
                                        // find next category for grouping end detection
                                        // note: since ordered by category then name, next element index +1 check works
                                        // we'll compute index by reference below
                                    endforeach; // placeholder to allow index-based loop below
                                endif;
                                // We'll render with an index-based loop so we can see next element easily
                                if (count($products) > 0):
                                    $prev_category = null;
                ...
                <!-- Cart & Payment -->
                <div class="col-md-5">
                    <form id="posForm" method="post" action="process_pos_branch1.php">
...
function renderCart() {
    ...
    cart.forEach(item => {
        ...
        total += subtotal;
        // Show product name (with unit), quantity input and a single amount (subtotal) to avoid duplication
    ...
    // Only show 'No items in cart.' if not in service-only mode
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            printBtn.onclick = function() {
                window.open('print_receipt_branch1.php?sale_id=' + data.sale_id, '_blank');
            };
        } else {
