...
$datalist_categories = [];
$cat_res = mysqli_query($conn, "SELECT DISTINCT category FROM products WHERE branch_id = 1");
while ($row = mysqli_fetch_assoc($cat_res)) {
    $datalist_categories[] = $row['category'];
}
...
<head>
    ...
    <title>Branch 1 Inventory</title>
...
<div class="container-fluid p-0 m-0">
    <div class="main-content">
        <h2 class="mb-2 mt-0 pt-0"><i class="bi bi-boxes me-2"></i>Branch 1 Inventory</h2>
        <div class="content-card">
            <form method="get" class="inventory-filters mb-3 d-flex gap-2 align-items-center">
                <input type="text" name="search" list="category-suggestions" value="<?php echo htmlspecialchars($search); ?>" placeholder="Search by category" class="form-control" style="max-width:220px;">
                <datalist id="category-suggestions">
                  <?php foreach ($datalist_categories as $cat): ?>
                ...
                <a href="?sort=<?php echo $sort === 'ASC' ? 'desc' : 'asc'; ?><?php echo $search ? '&search=' . urlencode($search) : ''; ?>" class="btn btn-outline-secondary">
                    Sort by Stock: <?php echo $sort === 'ASC' ? 'Ascending' : 'Descending'; ?>
                </a>
            </form>
                            ...
                            ?>
                                <tr<?php echo $classAttr . $styleAttr; ?>>
                                    ...
                                    <td class="text-center">
                                        <?php
                                        // Display unit: for Tires prefer the size field; for ml/L combine if needed
                                        $display_unit = '';
...
.inventory-table th.price-col,
.inventory-table td.price-col {
    text-align: right;
    padding-right: 18px;
}
.inventory-table tr {
...
.inventory-table th, .inventory-table td {
    border-right: 1px solid #eef2f6; /* subtle vertical separators between columns */
}

/* Category group visual separator: add a left border at the start of each category block */
.inventory-table tr.category-start td:first-child {
    border-left: 3px solid rgba(56, 161, 105, 0.12); /* soft green accent */
}

/* Ensure the separator row doesn't collapse the left border */
.inventory-table tr.category-separator td { border-top: none; border-bottom: none; }

/* Hover highlight: row only (horizontal) */
...
<!-- Hover highlight: highlight row only (horizontal) for better UX -->
<script>
(function(){
    ...
    rows.forEach(function(r){
        r.addEventListener('mouseenter', function(){ r.classList.add('row-highlight'); });
        r.addEventListener('mouseleave', function(){ r.classList.remove('row-highlight'); });
    });
    table.addEventListener('mouseleave', function(){ rows.forEach(function(r){ r.classList.remove('row-highlight'); }); });
})();
</script>
</body>
</html>
