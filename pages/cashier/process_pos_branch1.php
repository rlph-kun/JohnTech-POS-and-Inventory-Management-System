...
// Fetch mechanic name (if any)
$mechanic_name = '';
if ($mechanic_id) {
    $q = mysqli_query($conn, "SELECT name FROM mechanics WHERE id = $mechanic_id");
    if ($row = mysqli_fetch_assoc($q)) $mechanic_name = $row['name'];
}

// Fetch cashier name
$cashier_name = '';
$q = mysqli_query($conn, "SELECT name FROM cashier_profiles WHERE user_id = $cashier_id");
if ($row = mysqli_fetch_assoc($q)) $cashier_name = $row['name'];
