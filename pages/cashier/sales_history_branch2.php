...
// Fetch sales for branch 2 and date
$sql = "SELECT s.*, cp.name AS cashier_name, m.name AS mechanic_name FROM sales s
        ...
        WHERE s.branch_id = 2 AND DATE(s.created_at) = ? ORDER BY s.created_at DESC";
...
<head>
    ...
    <title>Juban Branch - Sales History</title>
...
<div class="container-fluid p-0 m-0">
    <div class="main-content">
        <h2 class="mb-2 mt-0 pt-0"><i class="bi bi-clock-history me-2"></i>Sales History - Juban Branch</h2>
        <div class="content-card">
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover align-middle">
                    ...
                    <tbody>
                        <?php if (empty($sales)): ?>
                            <tr><td colspan="11" class="text-center">No sales found.</td></tr>
                        <?php else: ?>
                            <?php foreach ($sales as $sale): ?>
                                <tr>
                                    ...
                                    <td><?= peso(max($payable, 0)) ?></td>
                                    <td><?= peso($sale['amount_received']) ?></td>
                                    <td><?= peso($sale['change_amount']) ?></td>
                                    <td>
                                        <a href="return_items.php?sale_id=<?= $sale['id'] ?>&branch_id=2" class="btn btn-warning btn-sm">
                                            <i class="bi bi-arrow-return-left"></i> Return
                                        </a>
                                    </td>
                                    <td><a href="print_receipt_branch2.php?sale_id=<?= $sale['id'] ?>" target="_blank" class="btn btn-outline-secondary btn-sm">Print</a></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </tbody>
